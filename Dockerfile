...
RUN apt-get -qq update -y && apt install -y software-properties-common \
    && apt-add-repository non-free && apt-get -qq update && apt-get -qq install -y --no-install-recommends \
    p7zip-full p7zip-rar unzip screenfetch xz-utils zstd tesseract-ocr libtesseract-dev wkhtmltopdf nodejs \
